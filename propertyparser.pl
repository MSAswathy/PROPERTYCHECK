...
getStateList(File,StList,TimeList) :-
    ...
    lex(S,X),
    close(S),parse(P,X,[]),
    predsort(comparelist,P,PL),extractstate(PL,StList),nl,write(StList),get_time_list(PL,TList),remove_duplicates(TList,TimeList),
                 nl,write(TimeList).
%--------------------------------------------------------------------------------------------------
extractstate([],[]).
...
%----------------------------------------------------------------------------------------------------
prop_check([]).
prop_check([H|T]):- H=id(X),check_var(X,Y),prop_check1([Y|T]).
prop_check1([H|T]) :- (H=G -> prop_g(T) ; prop_f(T)).

prop_g([H|T]) :- H=id(X),begintag(X),check_var(T,Var),getobject('logProp.txt',ObjList),
                 (memberchk(Var,ObjList) -> nl ; nl,print('Wrong Variable')),
                 check_var2(T,Var1),string_concat(Var,Var1,State),nl,write(State),
                 getStateList('logProp.txt',StList,TimeList),occurrences_of(StList, State, Count),nl,write(Count),
                 last(TimeList,Last),nl,write(Last),
                 (Count=Last -> print('Property VERIFIED AND ITS TRUE '); print('Property VERIFIED AND ITS FALSE ')).
                 
prop_f([H|T]) :- H=id(X),begintag(X),check_var(T,Var),getobject('logProp.txt',ObjList),
                (memberchk(Var,ObjList) -> nl ; nl,print('Wrong Variable')).
%-------------------------------------------------------------------------------------------------
check_var(X,Y) :- keyword(X),X=Y,nl,write(Y).
