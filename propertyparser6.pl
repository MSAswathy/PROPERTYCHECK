...
prop_check([X,Y|T],OL,SL,TL):- keyword(X),Y=id(BT),begintag(BT),last(T,ET),ET=id(E),endtag(E),
                                (X='G' -> (prop_g(T,OL,SL,TL),T=[T1,T2,T3,T4,T5,ET|Rest],ET=id(E),endtag(E)); 
                                (prop_f(T,OL,SL,TL),T=[T1,T2,T3,T4,T5,ET|Rest],ET=id(E),endtag(E))).
...
%--------------------IMPLIES Symbol----------------------------------------------------------
prop_checkforimplies([],[],[],[]):- !.
prop_checkforimplies([X|T],OL,SL,TL):- keyword(X),(X='G' -> (T=[A,B,C,D,E,id(Im),id(Key)|Rest],check_var([A,B,C],OVar),
                                       memberchk(OVar,OL),check_var2([D,E],SVar),string_concat(OVar,SVar,State),
                                       logop(Im),keyword(Key),
                                       prop_checkforimplies1(State,OL,SL,Rest));!,nl,nl,write('Wrong Syntax')),nl.
                                       
prop_checkforimplies1(_,[],[],[]):-!.                                       
prop_checkforimplies1(State,OL,SL,T):- traversestatechk(State,SL,Rest),T=[A,B,C,D,E|T1],
                                       check_var([A,B,C],OVar),
                                       memberchk(OVar,OL),check_var2([D,E],SVar),string_concat(OVar,SVar,State1),
                                       traversestatechk(State1,Rest),prop_checkforimplies1(State,State1,Rest).
prop_checkforimplies1(_,_,[]):-!.                                       
prop_checkforimplies1(State,State1,T):- traversestatechk(State,T,Rest),
                                        traversestatechk(State1,Rest),
                                        prop_checkforimplies1(State,State1,Rest).
...
prop_f([],[],[],[],[]):- !.
prop_f([H|T],OL,SL,TL,State,Final):- H='F',T=[X,Y,Z|T1],check_var([X,Y,Z],Var),memberchk(Var,OL),
                               check_var2(T1,Var1),string_concat(Var,Var1,State),
                               traversestatechk(State,SL,Final).
%-------------------------------------------------------------------------------------------------
traversestatechk(_, []):-!.
traversestatechk(X, [H | T]) :- member(X,H) -> !;traversestatechk(X,T). %Cut if found once
traversestatechk(X,[T]) :-  traversestatechk(X,T). %You look for all the subsequent cases

traversestatechk(_, [],[]):-!.
traversestatechk(X, [H|T],Rest) :- member(X,H) -> Rest=T,!;traversestatechk(X,T,Rest).
%-------------------------------------------------------
