getproperty(In):-
    ...
    getobject('log1sample.txt',ObjList),getStateList('log1sample.txt',StList,TimeList),
...
prop_checkfornegation([X|T],OL,SL,TL):- keyword(X),T=[A,B,C,D,E,id(ET),Z|T1],check_var([A,B,C],OVar),memberchk(OVar,OL),
                                        ...
                                        endtag(ET),logop(Z),
                                        prop_checkfornegation1([X|T1],OL,SL,TL,[State]).
prop_checkfornegation1([],[],[],[],[]):- !.                                       
prop_checkfornegation1([X|T],OL,SL,TL,ChkSt):- keyword(X),T=[id(BT),A,B,C,D,E,id(ET),Z|T1],
                                        begintag(BT),check_var([A,B,C],OVar),memberchk(OVar,OL),
                                        check_var2([D,E],SVar),string_concat(OVar,SVar,State),append(ChkSt,[State],Final),nl,write(Final),
                                        endtag(ET),logop(Z) ->
                                        prop_checkfornegation1([X|T1],OL,SL,TL,Final); !.   
...
%---getproperty('F  < NS=>C=red >'). True
%---getproperty('F  < NS=>C=green >').  True
%---getproperty('f  < NS=>C=green >').   Bad syntax
%---getproperty('G ~ < < NS=>C=green  > & < EW=>C=green  > >').
